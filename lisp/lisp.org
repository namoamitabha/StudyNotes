...
** TODO [LISP]Lisp quickstart
   ...
   2. Lisp II : http://cs.gmu.edu/~sean/lisp/LispTutorial2.html
	 ...
	 3. (mapcar function-pointer list)
	 4. (mapcar function-pointer list1 list2 ...)
	 5. If function-pointer points to a function which takes N arguments, then we must provide N lists
	 6. map, mapc, mapcan, mapcon ...
	 7. reduce, composing a function in on itself
	 8. (reduce function-pointer list)
	 9. function-pointer must point to a function which takes exactly two arguments
	 10. :from-end t keyword argument, change the order of operations
